...
from Tree.DecisionTreeFunction import DecisionTreeClassification
...
def RandomForestClassification(test_df, forest):
    """
    ...
    for i in range(len(forest)):
        column_name = "Tree_{}".format(i)
        classification = DecisionTreeClassification(test_df, tree = forest[i])
        df_classification[column_name] = classification

    df_classification = pd.DataFrame(df_classification)
    random_forest_classification = df_classification.mode(axis=1)[0]
