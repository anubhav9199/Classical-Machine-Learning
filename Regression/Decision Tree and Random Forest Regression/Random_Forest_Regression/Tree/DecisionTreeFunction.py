...
import numpy as np
import random
import Tree.HelperFunction as hf
...
def DecisionTreeAlgorithm(df, mltask, counter = 0, min_samples = 2, max_depth = 5, random_subspace = None):
...
def predict_example(example,tree):
    ...
    feature_name, comparison, value = question.split(sep = " ")
...
def DecisionTreePrediction(test_df, tree):
    """
    ...
    """
    prediction = test_df.apply(predict_example, args = (tree,), axis = 1)
    return prediction

def DecisionTreeClassification(test_df,tree):
    """
    test_df, tree
    ...
    """
    classification = test_df.apply(classify_example, args = (tree,), axis = 1)
...
"""def post_pruning(tree,df_train,df_val):
    ...
    errors_decision_node = sum(df_val.label != make_predictions)
